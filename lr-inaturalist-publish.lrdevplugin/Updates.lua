...
local function download(release, filename)
	...
	if LrFileUtils.exists(filename) and not LrFileUtils.isWritable(filename) then
		error(LOC("$$$/iNat/Updates/CannotWriteToFile=Cannot write to download file"))
	end
...
local function extract(filename, workdir)
	...
	if ret ~= 0 then
		error(LOC("$$$/iNat/Updates/CannotExtractFile=Could not extract downloaded release file"))
	end
end
...
local function downloadAndInstall(ctx, release)
	...
	if not r then
		error(LOC("$$$/iNat/Updates/CannotCreateTempDir=Cannot create temporary directory"))
	end
...
local function showUpdateDialog(release, force)
	...
	local toDo = LrDialogs.promptForActionWithDoNotShow({
		...
		verbBtns = {
			{ label = LOC("$$$/iNat/Updates/Download=Download"), verb = "download" },
			{ label = LOC("$$$/iNat/Updates/Ignore=Ignore"), verb = "ignore" },
		},
	})
...
function Updates.forceUpdate()
	LrTasks.startAsyncTask(function()
		...
		if v then
			LrDialogs.message(
				LOC("$$$/iNat/Updates/NoUpdatesAvailable=No updates available"),
				LOC("$$$/iNat/Updates/MostRecentVersion=You have the most recent version of the iNaturalist Publish Plugin, ^1", v),
				"info"
			)
		end
	end)
