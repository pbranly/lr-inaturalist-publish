...
local function clearObservation()
	...
	local confirmation = LrDialogs.confirm(
		...
		LOC("$$$/iNat/Clear/DeleteObservationData/Detail=This will clear the observation UUID and URL metadata fields from these photos")
	)
	...
	catalog:withWriteAccessDo("Clear observation", function(_)
		...
		local msg = LOC("$$$/iNat/Clear/RemovedObservationMetadata=Removed observation metadata from ^1 photos", #photos)
		if #photos == 1 then
			msg = LOC("$$$/iNat/Clear/RemovedObservationMetadataSingular=Removed observation metadata from ^1 photo", #photos)
		end
