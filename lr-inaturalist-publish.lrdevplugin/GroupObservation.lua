...
local function groupObservation()
		...
		if thisUUID and #thisUUID > 0 then
			if uuid ~= nil and uuid ~= thisUUID then
				LrDialogs.message(
					...
					LOC("$$$/iNat/Group/ConflictingMessage=Two photos in the selection already belong to different observations")
				)
				return
			end
			uuid = thisUUID
	...
	catalog:withWriteAccessDo(LOC("$$$/iNat/Group/GroupIntoObservation=Group into observation"), function(_)
		...
		local msg = LOC("$$$/iNat/Group/GroupedPhotos=Grouped %s photos into 1 observation")
